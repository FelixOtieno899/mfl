<template>
    <Main>
        <a-row :gutter="15">
            <a-col :xs="24">
                <BorderLessHeading>
                    <sdCards title="Manage Fuel Dispensation Orders">
                        <a-row>
                            <a-col
    ...
    setup() {
                    ...
                    if (response.status === 200) {
                        if (
                            ...
                            tableDataScource.value =
                                response.data.Fuel_dispensation_orders.map(
        ...
        const fetchFuelPurchaseOrders = async () => {
            try {
                ...
                console.log(
                    "Fuel Purchase Orders fetched",
                    response5.data.Fuel_purchase_orders,
                );
        ...
        const viewFuelDispensationOrder = (fuelDispensationOrderId) => {
            router.push(
                `/app/fuel-dispensation-order/${fuelDispensationOrderId}`,
            );
        };

        const editFuelDispensationOrder = (fuelDispensationOrderId) => {
            router.push(
                `/app/fuel-dispensation-order/edit/${fuelDispensationOrderId}`,
            );
        };
